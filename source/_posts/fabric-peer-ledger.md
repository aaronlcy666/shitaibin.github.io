...
交易并发执行到写入区块的过程中存在2种数据冲突的情况：
1. **写写冲突**：存在多个交易同时修改同一个key的情况。这种情况可以判断同一个区块中是否包含任何2笔交易的读集包含同一个key
1. **读写冲突**：执行交易T时读到了key，但写区块也在同时进行，为key写入了新版本的值，等交易T写账本时，发现交易T读到了老的key

第2种情况的交易因为读了老数据，一定无效。第1种情况，区块中第1笔改变某个key的交易是有效的，同一区块中第2笔修改相同key的交易会是无效的。

有效交易的写集会被应用到世界状态数据库，被修改数据都会有一个新的版本。
